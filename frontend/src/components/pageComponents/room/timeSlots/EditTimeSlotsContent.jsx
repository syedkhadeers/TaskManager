...
const EditTimeSlotsContent = ({ timeSlot, onClose, onTimeSlotUpdated }) => {
  ...
  useEffect(() => {
    if (timeSlot) {
      setFormData({
        ...
        sameDay: timeSlot.sameDay || "SameDay",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await updateTimeSlot(timeSlot.id, formData);
      if (response) {
        toast.success("Time slot updated successfully!");
        onTimeSlotUpdated && onTimeSlotUpdated(response);
        onClose();
      }
    } catch (error) {
    ...
    >
      <div className="p-6">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {isLoading ? "Updating..." : "Update Time Slot"}
          </button>
        </form>
      </div>
    </motion.div>
